...
rotation_speeds_all=[[math.pi/detail*r*i for r in axes_of_rotation] for i in [1 if i % 2 == 0 else -1 for i in range(10)]]
render=[]
x=0
no=True
colrot1=0
...
def half_size_cube(vertices):
    center=[450,450,600]
    half_side_length=min([abs(coord-center[i])for i,coord in enumerate(vertices[0])])/2
    new_vertices=[[center[i]+(coord-center[i])/2 for i,coord in enumerate(vertex)] for vertex in vertices]
    return new_vertices
